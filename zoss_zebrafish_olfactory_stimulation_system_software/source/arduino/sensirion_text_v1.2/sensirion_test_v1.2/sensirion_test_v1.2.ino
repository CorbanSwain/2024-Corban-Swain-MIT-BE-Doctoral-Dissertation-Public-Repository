...
const unsigned long BAUD_RATE = 576000;
...
void setup() {
  ...
  for(int i = 0; i < MAX_NUM_PORTS; i++) {    
    comm_check = false;
    safe_select_tca_port(i,  comm_check);
...
void loop() {
  ...
  for (int i = 0; i < MAX_NUM_PORTS; i++) {
    ...
    raw_value = sensirion_read_int16(flow_values_failed);

    if (comm_check) {
      flow_value = 0;
    } else {
      flow_value = ((float) raw_value) / FLOW_SCALE_FACTORS[sensors[i]];
    }

    temp_value_failed = false;
    raw_value = sensirion_read_int16(temp_value_failed);

    if (temp_value_failed) {
      temp_value = 0;
    } else {
      temp_value = ((float) raw_value) / SCALE_FACTOR_TEMP;
    }

    flag_values_failed = false;
    sensirion_read_measurement_flags(
      air_in_line_flag, high_flow_flag, exp_smooth_flag, flag_values_failed);
